...
> **2026-09-14 已归档**:步 1–3 落地(用户确认);步 4「中转后端推算」未做,挪到总览 `../../plan/2026-09-14/main-tasks.md` §2.3 未排期,原设计留在本文 §2.C、§4。
...
> 3. claude-code / codex / agy 这类中转后端,顾清影 看不到 CLI 自己的系统提示词和原生工具;能给的是
>    「顾清影 发过去的部分」分项 + 「CLI 自带」**推算**一行,标明是推算，但是市面上有这类中转后段的真实数据获取。
...
- 会话归属校验:与 `../../plan/2026-09-14/job-control.md` §A 抽出的判定函数同一个。

### C. 中转后端

顾清影 通过 MCP 把工具交给 CLI,CLI 再叠上自己的系统提示词与原生工具,顾清影 看不到这部分。

- 分项照 A 算「顾清影 发过去的部分」。
- 多一行 **「CLI 自带(推算)」= 单次请求实测 input − 顾清影 估算合计**,标明推算、包含分词器差异。
- 窗口大小:CLI 实际窗口与 顾清影 配置可能不同。能从事件里拿到就用事件里的,拿不到沿用配置并标「按配置」。
...
用户要先看前端调试:单文件原型放在 `~/Desktop/gqy-context-panel.html`(**不进仓库**),假数据,样式取自 `web/styles.css` 的真实 token。
