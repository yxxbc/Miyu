# 中转线统一走 顾清影 压缩（2026-09-10）

用户裁定：claude-code / codex / antigravity 三条中转线**统一用 顾清影 自己的压缩**，不依赖 CLI 那头的自压缩。理由是统一、可控。
...
三条线的工具事件（原生工具与 `mcp__gqy__*` 桥工具都一样）翻成 `RemoteToolStarted/Finished`，
...
活库证据（`~/.gqy/state/conversation.db` 副本）：42 个 remote 轮（claude-code 21、antigravity 21）footprint 全空；
antigravity 那 21 轮里有 `view_file`×4、`write_to_file`×4，真读真写了也是空。
bda27508 修的 `edit`/`patchText` 只救了直连线。

### 2. claude-code 线上 顾清影 与 CLI 两套压缩抢跑

`builder.rs` 把 顾清影 有效窗口透传成 `--autocompact <window>`。两条触发线量的是同一个数（最近一次 API usage）：

| 窗口 W | 顾清影 0.8×W | claude W−20k−13k | 谁先 |
|---|---|---|---|
| 100k | 80,000 | 67,000 | claude |
| 168k（默认） | 134,400 | 135,000 | 顾清影，早 600 tok |
| 200k | 160,000 | 167,000 | GQY |

默认下 顾清影 先到；一压，哈希链断、CLI 会话重开全量重放，claude 的 autocompact 从没跑过。
...
codex 有 `model_auto_compact_token_limit`/`model_context_window` 键但 顾清影 没传；agy 有内部压缩无外部旋钮。
...
只要 顾清影 的 `context_window` 配置不大于 CLI 的真实窗口，顾清影 的 0.8 线永远先到。
...
- 真机（`testkit/relay-compact/relay_probe.py`，跑完打 PASS/FAIL）：隔离 GQY_HOME + 真 claude-code(haiku)，dev 会话 Read+Edit、Write 各一轮
  → `gqy compact` → 查 `turns.tool_footprint`、摘要尾 `<read-files>/<modified-files>`、`compact_extras.restored`。
...
| claude 进程参数（单测 `relay_leaves_compaction_to_gqy` 抓 args.txt） | `--autocompact <窗口>` | 无 |
