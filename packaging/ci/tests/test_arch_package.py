...
class ArchPackageTests(unittest.TestCase):
    def test_generated_recipe_has_exact_voice_dependency_and_rejects_shell_text(self):
        self.assertIn("'gqy=0.6.0-1'", recipe('0.6.0', 1, 'voice'))
    ...
    def test_source_recipes_install_from_the_shared_catalog(self):
        with tempfile.TemporaryDirectory(prefix='gqy-source-recipe-') as name:
            root = Path(name)
            ...
            (source/'target/x86_64-unknown-linux-gnu/release/gqy').write_bytes(b'fixture binary')
            ...
            (source/'packaging/common/assets.json').write_text(json.dumps({
                'schema_version': 1, 'assets': [{
                    ...
                    'destination': 'share/gqy/fonts/font.ttf', 'mode': '0644',
                    'component': 'core', 'type': 'file'}, {
                    'id': 'wiki-commit', 'source_root': 'runtime',
                    'source': 'default-kb/manifest/shorinwiki.commit',
                    'destination': 'share/gqy/default-kb/manifest/shorinwiki.commit',
            ...
            self.assertEqual((destination/'share/gqy/fonts/font.ttf').read_bytes(), b'catalog-selected bytes')
            self.assertEqual((destination/'share/gqy/default-kb/manifest/shorinwiki.commit').read_text(), revision+'\n')
            self.assertEqual(os.readlink(destination/'bin/gqypm'), 'gqy')
    ...
    def test_binary_wrapper_preserves_alias_and_complete_resource_tree(self):
        with tempfile.TemporaryDirectory(prefix='gqy-repackage-') as name:
            root = Path(name)
            source, destination = root/'src', root/'pkg'
            (source/'usr/bin').mkdir(parents=True)
            (source/'usr/share/gqy/fonts').mkdir(parents=True)
            (source/'usr/bin/gqy').write_bytes(b'fixture binary')
            (source/'usr/bin/gqypm').symlink_to('gqy')
            (source/'usr/share/gqy/fonts/font.ttf').write_bytes(b'font fixture')
            ...
            subprocess.run(['bash', '-euc', 'source "$1"; package', 'test',
                str(REPO/'packaging/arch/gqy/PKGBUILD')], env=env, check=True, timeout=30)
            self.assertTrue((destination/'usr/bin/gqypm').is_symlink())
            self.assertEqual(os.readlink(destination/'usr/bin/gqypm'), 'gqy')
            self.assertEqual((destination/'usr/share/gqy/fonts/font.ttf').read_bytes(), b'font fixture')
