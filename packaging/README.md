...
| 目录 | 用途 |
|---|---|
| `arch/gqy-git/` | AUR VCS 包，继续从 Git 源码构建；`pkgver()` 读取实际版本和提交 |
| `arch/gqy-release/` | 从普通版本 tag，或明确声明的补丁提交，构建主包/voice 拆包 |
| `arch/gqy/` | AUR 主包包装器，下载已发布的 Arch 主包后完整重包 |
| `arch/gqy-voice/` | AUR voice 包装器，依赖与下载资产一致的主包精确版本 |
...
`gqy-release` 默认构建 `v0.6.0`，Wiki 固定为
`af99c4ac22a1be849206639807577a51b9e12061`。本地安装验收使用冻结源码快照的原生构建，
不需要先创建公开 tag。若发布资产确实使用 tag 以外的补丁，必须明确提供
`GQY_RELEASE_SOURCE_COMMIT`、`GQY_RELEASE_PATCH_REASON`，并记录到 release input；
...
包装器复制整个 `usr/`，因此保留 `gqypm` 别名、字体、资源、许可证和未来新增文件。
...
`share/gqy`、Linux 系统 fallback、仅 debug 的源码 fallback。release 不读当前目录
伪造的 `src/memes` 或 `assets/models`。所有相对路径均相对安装 prefix；Arch 为 `/usr`。

| 路径 | 内容 | 缺失时的行为 |
|---|---|---|
| `bin/gqy`、`bin/gqypm` | 主程序及别名 | 主程序不可用 |
| `bin/gqy-voice` | 可选语音前端 | 语音不可用 |
| `share/gqy/fonts/` | Noto CJK、Noto Emoji、JetBrains Mono；发布资产已包含 | 渲染可能退回文本 |
| `share/gqy/models/<id>/` | 本地 embedding 模型及 tokenizer | 语义检索退回关键词 |
| `share/gqy/memes/` | 内置表情库 | 内置表情不可用 |
| `share/gqy/scripts/` | 内置脚本 | 对应脚本不可用 |
| `share/gqy/default-kb/` | 项目 kb、固定 Wiki 和来源 manifest | 默认知识库不可用 |
| `share/licenses/gqy*/` | 项目、字体、模型及语音依赖许可证 | 包验收失败 |

GNU 渠道还在 `lib/gqy/` 携带固定 CPU ORT；Arch 继续使用系统 provider。
