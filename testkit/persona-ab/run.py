...
def run_tool_matrix():
    ...
    for index, (tag, acfg, probe) in enumerate(plan, 1):
        ...
        env = {"GQY_STYLE_LOCK": "1" if acfg["style_lock"] else "0"}
        try:
...
def score_chat(reply: str) -> dict:
    """默认 顾清影 人格显式规则的机械化子集(与 08-14 判据同源):
